...
class MotovisObjectDataPrepare():
    """
    Function:
        Prepare data infos used for refining module
    """
    def __init__(self, class_name, root_path=None, 
                 ouput_path=None,
                 split='train', track_data_path=None,
        ...
        self.save_path = dict()
        for cn in self.class_name:
            if ouput_path is None:
                self.save_path[cn] = os.path.join(root_path, 'refining', split, cn)
            else:
                self.save_path[cn] = os.path.join(ouput_path, 'refining', split, cn)

            if os.path.exists(self.save_path[cn]):
                os.system('rm -rf {}'.format(self.save_path[cn]))
            os.makedirs(self.save_path[cn])
    ...
    def prepare_data_worker(self, seq_dict):
        ...
        save_path = dict()
        for cn, sp in self.save_path.items():
            save_path[cn] = os.path.join(sp, '%s.pkl' % seq)
        ...
        if self.split in ['train', 'val']:
            ...
            for tk_id in tracklet_ids:
                ...
                name = tk_infos['name'] # is array, all are same
                # pdb.set_trace()
                ...
                pose = tk_infos['pose']
                # pdb.set_trace()
                lidar_paths = tk_infos['lidar_path']
                ...
                for idx, frm_id in enumerate(sample_idx):
                    if name[idx] not in self.class_name:
                        continue
        ...
        frame_ids = list(output_dict.keys())
        data_info = {}
        for frm_id in tqdm(frame_ids, desc=seq, leave=True):
            ...
            if len(frm_info['boxes_global']) > 0:
                ...
                lidar_path = os.path.join(self.root_path, str(frm_info['lidar_path']))
                # pdb.set_trace()
                pts = np.fromfile(lidar_path, dtype=np.float32).reshape(-1, self.pt_dim)[:, :3]
                ...
                # pose from tracking, is ego2global or lidar2global
        ...
        for obj_id in data_info:
            ...
            if obj_info['name'] not in data_info_cn.keys():
                data_info_cn[obj_info['name']] = dict()
            data_info_cn[obj_info['name']][obj_id] = data_info[obj_id]
        for cn, data_info in data_info_cn.items():
            # save the generated infos
            with open(save_path[cn], 'wb') as f:
...
if __name__ == '__main__':
    ...
    # class_names = ["car", "truck", "construction_vehicle", "bus"]
    
    logger.info('Start to process %s data ...' % class_names)
    dataset = MotovisObjectDataPrepare(
        class_name=class_names,
        # root_path=os.path.join(ROOT_DIR, 'data', 'waymo'),
        root_path=args.root_dir,
        split=args.split,
        ouput_path=args.output_dir,
