...
from ...utils.transfusion_utils import clip_sigmoid, PositionEmbeddingLearned, TransformerDecoderLayer
...
from detzero_det.utils import model_nms_utils, centernet_utils
import pdb
...
class TransFusionHead(nn.Module):
    ...
    def predict(self, inputs):
        ...
        # elif self.dataset_name == 'ontime':
        #     # for Pedestrian & Traffic_cone in ontime
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d, preds_dict):
        ...
        feature_map_size = (self.grid_size[:2] // self.feature_map_stride) # (x, y)
        ...
        for idx in range(len(gt_bboxes_3d)):
            ...
            if width > 0 and length > 0:
                ...
                center = torch.tensor([coor_y, coor_x], dtype=torch.float32, device=device)
    ...
    def get_bboxes(self, preds_dicts):
        ...
        if self.dataset_name == "nuScenes":
            ...
            self.tasks = [
                dict(num_class=5, class_names=[], indices=[0, 1, 2, 3, 4, 5, 7], radius=-1),
                dict(num_class=1, class_names=["pedestrian"], indices=[6], radius=0.175),
                dict(num_class=1,class_names=["traffic_cone"],indices=[8],radius=0.175),
            ]
