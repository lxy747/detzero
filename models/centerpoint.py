...
class CenterPoint(nn.Module):
    ...
    def post_process_ego_fp(self, pred_boxes, pred_scores, pred_labels):
        if pred_boxes.shape[0]==0:
            return pred_boxes, pred_scores, pred_labels
        ego_w = 1.9
        ego_l = 5.2
        ego_h = 2.2
        ego_bb = torch.zeros_like(pred_boxes[:1, :7])
        if self.dataset.dataset_cfg.REF_CS_IS_LIDAR:
            # rfu
            ego_bb[0, 3] = ego_w
            ego_bb[0, 4] = ego_l
            ego_bb[0, 5] = ego_h
            ...
            ego_bb[0, 5] = ego_h
            ego_bb[0, 6] = 0.0
        aiou = iou3d_nms_utils.boxes_iou_bev(ego_bb, pred_boxes[:, :7])[0]
        valid = aiou < 0.0001
    ...
    def post_processing(self, batch_dict):
        ...
        if self.second_stage:
            ...
            for index in range(batch_size):
                ...
                # final_boxes, final_scores, final_labels = self.post_process_ego_fp(final_boxes, final_scores, final_labels)
            ...
            for index in range(batch_size):
                final_boxes, final_scores, final_labels = self.post_process_ego_fp(**pred_dicts[index])
                pred_dicts[index].update({
                    'pred_boxes': final_boxes,
                    'pred_scores': final_scores,
                    'pred_labels': final_labels
                })
