...
class NuScenesDataset(DatasetTemplate):
    ...
    def get_gt(self, info):
        ...
        if 'anns' in info:
            ...
            tmp_info = info
        if 'gt_boxes' not in tmp_info.keys():
            return gt
        if self.dataset_cfg.get('FILTER_MIN_POINTS_IN_GT', False) and 'num_lidar_pts' not in tmp_info.keys():
            mask = (tmp_info['num_lidar_pts'] > self.dataset_cfg.FILTER_MIN_POINTS_IN_GT - 1)
        else:
            mask = None

        gt_boxes = np.array(tmp_info['gt_boxes'])
        if ('gt_velocity_3d' in tmp_info.keys()) and (self.dataset_cfg.PRED_VELOCITY):
            # import pdb; pdb.set_trace()
            # gt_boxes = np.concatenate([gt_boxes, tmp_info['gt_velocity']], axis=1).copy()
            gt_boxes = np.concatenate([gt_boxes, np.array(tmp_info['gt_velocity_3d'])[:, :2]], axis=1).copy()


        gt.update({
            'gt_names': tmp_info['gt_names'] if mask is None else tmp_info['gt_names'][mask],
