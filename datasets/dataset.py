...
class DatasetTemplate(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None) -> None:
        ...
        self.REF_CS_IS_LIDAR = dataset_cfg.get('REF_CS_IS_LIDAR', True)
    ...
    def names2id(self, names):
        return [self.class_names.index[n] + 1 for n in names]
    
    def id2names(self, name_ids):
        return [self.class_names(i-1) for i in name_ids]
