...
class DataAugmentor(object):
    ...
    def random_points_mask(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.random_points_mask, config=config)
        points = data_dict['points']
        max_ratio = config.get('MAX_MASK_RATIO', 0.1)
        n = points.shape[0]
        mask = np.random.rand(n)
        thr = np.random.uniform(0, 1) * max_ratio
        mask = mask > thr
        points = points[mask, :]
        data_dict['points'] = points
        return data_dict
...
class MotovisDataAugmentor(DataAugmentor):
    def gt_sampling(self, config=None):
        db_sampler = database_sampler.MotovisDataBaseSampler(
            root_path=self.root_path,
            sampler_cfg=config,
            class_names=self.class_names,
            logger=self.logger
