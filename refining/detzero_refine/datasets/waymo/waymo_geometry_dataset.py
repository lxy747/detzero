...
class WaymoGeometryDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        # pdb.set_trace()
        if training and 'QUERY_NUM_TRAIN' in dataset_cfg.keys():
            self.query_num = self.dataset_cfg.get('QUERY_NUM_TRAIN')
        else:    
    ...
    def revert_to_each_frame(data_dict):
        ...
        for i, pred_box in enumerate(data_dict['pred_boxes']):
            ...
            pose = data_dict['pose'][i].astype(np.float32) # lidar2global
...
class MotovisGeometryDataset(WaymoGeometryDataset):

    def init_class_map(self):
        class_names = ["car", "truck", "construction_vehicle", "bus", 
                       "bicycle", "tricycle", "pedestrian", "barrier", "traffic_cone", "generic_object"]
        self.class_map = dict()
        for i, cls in enumerate(class_names):
            self.class_map[cls]=i
            self.class_map[i] = cls
        # self.class_map = {
        #     'Vehicle': 1, 'Pedestrian': 2, 'Cyclist': 3,
        #     1: 'Vehicle', 2: 'Pedestrian', 3: 'Cyclist'
        # }
    
    def load_sample_sequence_list(self):
        self.root_path = os.path.join(self.root_path, self.split)
    ...
    def load_track_infos(self):
        ...
        for cls_name in self.class_names:
            ...
            data_path = os.path.join(self.root_path, cls_name)
            file_names = os.listdir(data_path)

            tmp_seq_ids = [os.path.join(data_path, seq) for seq in file_names]
            seq_ids.extend(tmp_seq_ids)
        ...
        if self.iou:
            ...
            iou_path = os.path.join(self.root_path, '..', self.iou)
            with open(iou_path, 'rb') as f:
                self.iou = pickle.load(f)
        # self.workers_num = 0
        # res = multi_processing(self.load_infos_worker, seq_ids, self.workers_num, bar=True)
        res = []
        for seq_name in tqdm(seq_ids):
            res.append(self.load_infos_worker(seq_name))
        for item in res:
