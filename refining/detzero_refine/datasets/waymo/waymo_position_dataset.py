...
from .waymo_geometry_dataset import MotovisGeometryDataset
import pdb
class WaymoPositionDataset(DatasetTemplate):
    ...
    def extract_track_feature(self, data_info):
        ...
        for enc_cfg in self.encoding:
            ...
            elif enc_cfg == 'intensity': # 1
                local_pts_data.append(query_pts[:, :, [3]])
                global_pts_data.append(traj_pts[:, :, [3]])

            elif enc_cfg == 'p2co': # 27
                # import pdb; pdb.set_trace()
                co_pts = boxes_to_corners_3d(traj).reshape(box_num, -1) # [box_num, 24]
                co_ce_pts = np.concatenate([co_pts, traj[:, :3]], axis=-1) # [box_num, 27]
    ...
    def revert_to_each_frame(self, data_dict):
        ...
        for i in range(len(data_dict['pred_boxes'])):
            ...
            pose = data_dict['pose'][i].astype(np.float32)
            frm_len = len(pose)
...
class MotovisPositionDataset(MotovisGeometryDataset):
    def extract_track_feature(self, data_info):
        # import pdb; pdb.set_trace()
        obj_cls = self.class_map[data_info['name']]
        traj_all = data_info['boxes_global']
        ...
        for enc_cfg in self.encoding:
            if enc_cfg == 'placeholder':
                ...
                tmp_cat = np.zeros(len(self.class_map)//2)
                tmp_cat[obj_cls - 1] = 1
                local_pts_data.append(np.tile(tmp_cat[None, None, :], (box_num, self.query_pts_num, 1)))
                global_pts_data.append(np.tile(tmp_cat[None, None, :], (box_num, self.memory_pts_num, 1)))
        ...
        if self.training:
            ...
            query_num = max(box_num, self.query_num)
        # pad the box
        local_pts_data = np.concatenate([
            local_pts_data,
            np.zeros((query_num-box_num, self.query_pts_num, local_pts_data.shape[2]))],
            axis=0
        )
        global_pts_data = np.concatenate([
            global_pts_data,
            np.zeros((query_num-box_num, self.memory_pts_num, global_pts_data.shape[2]))],
            axis=0
        )

        zeros = np.zeros((query_num-len(traj_gt), 7), dtype=np.float32)
        traj_gt = np.concatenate((traj_gt[:, :7], copy.deepcopy(zeros)), axis=0)
        traj = np.concatenate((traj[:, :7], copy.deepcopy(zeros)), axis=0)
        padding_mask = np.concatenate(
            (np.zeros(box_num), np.full(query_num-box_num, 1)), axis=0)
    ...
    def generate_prediction_dicts(self, batch_dict, pred_dicts, single_pred_dict, output_path=None):
        ...
        for per_obj in range(len(all_pred_res)):
            ...
            for idx, frm_id in enumerate(batch_dict['frame'][per_obj]):
                ...
                single_pred_dict[seq][obj_id]['name'].append(self.class_map[int(batch_dict['obj_cls'][per_obj])])
                single_pred_dict[seq][obj_id]['pose'].append(pred_dicts['pose'][per_obj][idx])
                single_pred_dict[seq][obj_id]['frame_id'].append(frm_id)
                single_pred_dict[seq][obj_id]['boxes_global'].append(all_pred_world_res[per_obj][idx])
                if len(all_gt_res)>0:
                    single_pred_dict[seq][obj_id]['boxes_gt'].append(all_gt_res[per_obj][idx])
                    single_pred_dict[seq][obj_id]['boxes_gt_global'].append(all_gt_world_res[per_obj][idx])
    ...
    def revert_to_each_frame(self, data_dict):
        ...
        for i in range(len(data_dict['pred_boxes'])):
            ...
            # the same process for gt_box
            if 'gt_pos_trajectory' in data_dict.keys():
                box_gt_world = data_dict['gt_pos_trajectory'][i].copy()
                box_gt_world = box_coords_transform(box_gt_world, init_box)
                seq_world_gt.append(box_gt_world[:frm_len, :].copy())
    ...
    def collate_batch(batch_list, _unused=False):
        # pdb.set_trace()
        query_num = [data['pos_query_points'].shape[0]  for data in batch_list]
        max_q = np.max(query_num)
        for data in batch_list:
            qn = data['pos_query_points'].shape[0]
            if qn>=max_q: continue
            pad_size = max_q - qn
            # pdb.set_trace()
            data['padding_mask'] = np.pad(data['padding_mask'], pad_width=(0, pad_size), mode='constant', constant_values=1)
            data['pos_query_points'] = np.pad(data['pos_query_points'], pad_width=((0, pad_size), (0, 0), (0, 0)), mode='constant', constant_values=0)
            data['pos_memory_points'] = np.pad(data['pos_memory_points'], pad_width=((0, pad_size), (0, 0), (0, 0)), mode='constant', constant_values=0)
            data['pos_trajectory'] = np.pad(data['pos_trajectory'], pad_width=((0, pad_size), (0, 0)), mode='constant', constant_values=0)
            if 'gt_pos_trajectory' in data.keys():
                data['gt_pos_trajectory'] = np.pad(data['gt_pos_trajectory'], pad_width=((0, pad_size), (0, 0)), mode='constant', constant_values=0)

        return DatasetTemplate.collate_batch(batch_list=batch_list, _unused=_unused)
