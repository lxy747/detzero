...
class MotovisConfidenceDataset(MotovisGeometryDataset):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        super().__init__(dataset_cfg, class_names, training, root_path, logger)
    ...
    def extract_track_feature(self, data_info):
        ...
        input_pts_data = np.concatenate([
            input_pts_data,
            np.zeros((query_num-box_num, self.query_pts_num, input_pts_data.shape[2]))],
            axis=0
        )
        # import pdb; pdb.set_trace()
        # pad to a same track length
        iou = np.concatenate((iou, np.full(query_num-len(iou), -1)), axis=0)
        score = np.concatenate((score, np.full(query_num-len(score), -1)), axis=0)
    ...
    def collate_batch(batch_list, _unused=False):
        # import pdb; pdb.set_trace()
        query_num = [data['conf_points'].shape[0]  for data in batch_list]
        max_q = np.max(query_num)
        for data in batch_list:
            qn = data['conf_points'].shape[0]
            if qn>=max_q: continue
            pad_size = max_q - qn
            # pdb.set_trace()
            data['conf_points'] = np.pad(data['conf_points'], pad_width=((0, pad_size), (0, 0), (0, 0)), mode='constant', constant_values=0)
            data['conf_score'] = np.pad(data['conf_score'], pad_width=(0, pad_size), mode='constant', constant_values=-1)
            data['iou'] = np.pad(data['iou'], pad_width=(0, pad_size), mode='constant', constant_values=-1)
